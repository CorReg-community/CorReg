...
#' @param clean (boolean) to decide either or not variables names has to be 
#' cleaned (spaces, accents and special characters)
#' @param keep (boolean) to decide either or not events and group are to 
#' be added to the resulting data.frame
#' @return a data.frame which number of rows is the length of events
#' 
#' @examples 
...
#' 
#' @export
counting_events_group <- function(events = events, group = NULL, base = NULL, prefix = NULL, suffix = NULL, clean = TRUE, keep = FALSE) {
    # base=base events=base$cat_aci group=base$NO_PRG table(base$cat_aci,base$fam_pro_vis)
    if (is.null(group)) {
    ...
    if (clean == TRUE) {
        names(additional) = clean_symbols(names(additional))
    }
    
    # define a variable to store local Montage (inital value = first met after timestamp ordering)
    local_prog = group[1]
    
    ## Core code For each coil
    for (i in 2:nrow(additional)) {
        ...
        if (group[i] != group[i - 1]) {
            ...
            # else (same montage) => copy previous line
    ...
    if (keep == FALSE) {
        # don't keep columns used for counting
